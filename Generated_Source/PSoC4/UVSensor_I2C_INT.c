...
/*******************************************************************************
* Function Name: UVSensor_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(UVSensor_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef UVSensor_I2C_ISR_ENTRY_CALLBACK
    UVSensor_I2C_ISR_EntryCallback();
#endif /* UVSensor_I2C_ISR_ENTRY_CALLBACK */

#if (UVSensor_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = UVSensor_I2C_ACK_ADDR;
    ...
    if(NULL != UVSensor_customIntrHandler)
    {
        UVSensor_customIntrHandler();
    }

    if(UVSensor_CHECK_INTR_I2C_EC_MASKED(UVSensor_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(UVSensor_CHECK_I2C_FSM_MASTER)
    {
        #if(UVSensor_I2C_MASTER)
        {
            ...
            */
            if(UVSensor_CHECK_INTR_MASTER_MASKED(UVSensor_INTR_MASTER_I2C_BUS_ERROR))
            {
                UVSensor_mstrStatus |= (uint16) (UVSensor_I2C_MSTAT_ERR_XFER |
                                                         UVSensor_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = UVSensor_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(UVSensor_CHECK_INTR_MASTER_MASKED(UVSensor_INTR_MASTER_I2C_ARB_LOST))
            {
                UVSensor_mstrStatus |= (uint16) (UVSensor_I2C_MSTAT_ERR_XFER |
                                                         UVSensor_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = UVSensor_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(UVSensor_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(UVSensor_CHECK_I2C_MASTER_CMD(UVSensor_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    UVSensor_mstrStatus |= (uint16) (UVSensor_I2C_MSTAT_ERR_XFER |
                                                             UVSensor_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                UVSensor_mstrStatus |= (uint16) UVSensor_GET_I2C_MSTAT_CMPLT;

                #if(UVSensor_I2C_MULTI_MASTER_SLAVE)
                {
                    if(UVSensor_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(UVSensor_CHECK_I2C_MASTER_CMD(UVSensor_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            UVSensor_I2C_MASTER_CLEAR_START;

                            endTransfer = UVSensor_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!UVSensor_CHECK_INTR_SLAVE_MASKED(UVSensor_INTR_SLAVE_I2C_BUS_ERROR))
                               && UVSensor_CHECK_INTR_MASTER_MASKED(UVSensor_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = UVSensor_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            UVSensor_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            UVSensor_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            UVSensor_state = UVSensor_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (UVSensor_CHECK_I2C_FSM_SLAVE) */
    {
        #if(UVSensor_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or UVSensor_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(UVSensor_CHECK_INTR_SLAVE_MASKED(UVSensor_INTR_SLAVE_I2C_BUS_ERROR |
                                                        UVSensor_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(UVSensor_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    UVSensor_slStatus &= (uint8) ~UVSensor_I2C_SSTAT_RD_BUSY;
                    UVSensor_slStatus |= (uint8) (UVSensor_I2C_SSTAT_RD_ERR |
                                                          UVSensor_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    UVSensor_slStatus &= (uint8) ~UVSensor_I2C_SSTAT_WR_BUSY;
                    UVSensor_slStatus |= (uint8) (UVSensor_I2C_SSTAT_WR_ERR |
                                                          UVSensor_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(UVSensor_CHECK_I2C_FSM_MASTER)
    {
        #if(UVSensor_I2C_MASTER)
        {
            ...
            */
            if(UVSensor_CHECK_INTR_MASTER_MASKED(UVSensor_INTR_MASTER_I2C_STOP))
            {
                UVSensor_ClearMasterInterruptSource(UVSensor_INTR_MASTER_I2C_STOP);
            ...
            {
                if(UVSensor_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(UVSensor_CHECK_INTR_MASTER_MASKED(UVSensor_INTR_MASTER_I2C_NACK))
                    {
                        UVSensor_ClearMasterInterruptSource(UVSensor_INTR_MASTER_I2C_NACK);

                        UVSensor_mstrStatus |= (uint16) (UVSensor_I2C_MSTAT_ERR_XFER |
                                                                 UVSensor_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(UVSensor_CHECK_I2C_FSM_RD) /* Reading */
                        {
                            UVSensor_state = UVSensor_I2C_FSM_MSTR_RD_DATA;
                        }
                        else /* Writing */
                        {
                            UVSensor_state = UVSensor_I2C_FSM_MSTR_WR_DATA;
                            if(0u != UVSensor_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                UVSensor_SetTxInterruptMode(UVSensor_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(UVSensor_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(UVSensor_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(UVSensor_CHECK_INTR_RX_MASKED(UVSensor_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  UVSensor_mstrRdBufSize -
                                        (UVSensor_mstrRdBufIndex + UVSensor_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > UVSensor_I2C_FIFO_SIZE)
                            {
                                diffCount = UVSensor_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    UVSensor_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                UVSensor_mstrRdBufPtr[UVSensor_mstrRdBufIndex] = (uint8)
                                                                                        UVSensor_RX_FIFO_RD_REG;
                                UVSensor_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(UVSensor_CHECK_INTR_RX_MASKED(UVSensor_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            UVSensor_mstrRdBufPtr[UVSensor_mstrRdBufIndex] = (uint8) UVSensor_RX_FIFO_RD_REG;
                            UVSensor_mstrRdBufIndex++;

                            if(UVSensor_mstrRdBufIndex < UVSensor_mstrRdBufSize)
                            {
                                UVSensor_I2C_MASTER_GENERATE_ACK;
                            }
                            else
                            {
                               endTransfer = UVSensor_I2C_CMPLT_ANY_TRANSFER;
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(UVSensor_CHECK_INTR_MASTER_MASKED(UVSensor_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            UVSensor_mstrWrBufIndexTmp -= (UVSensor_GET_TX_FIFO_ENTRIES +
                                                                   UVSensor_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            UVSensor_mstrWrBufIndex = UVSensor_mstrWrBufIndexTmp;

                            UVSensor_mstrStatus |= (uint16) (UVSensor_I2C_MSTAT_ERR_XFER |
                                                                     UVSensor_I2C_MSTAT_ERR_SHORT_XFER);

                            UVSensor_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(UVSensor_CHECK_INTR_TX_MASKED(UVSensor_INTR_TX_EMPTY))
                        {
                            while(UVSensor_I2C_FIFO_SIZE != UVSensor_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(UVSensor_mstrWrBufIndexTmp < UVSensor_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(UVSensor_mstrWrBufIndexTmp == (UVSensor_mstrWrBufSize - 1u))
                                    {
                                        UVSensor_ClearTxInterruptSource(UVSensor_INTR_TX_UNDERFLOW);
                                        UVSensor_SetTxInterruptMode(UVSensor_INTR_TX_UNDERFLOW);
                                    }
                                 #endif /* (!UVSensor_CY_SCBIP_V0) */

                                    /* Put data into TX FIFO */
                                    UVSensor_TX_FIFO_WR_REG = (uint32) UVSensor_mstrWrBufPtr[UVSensor_mstrWrBufIndexTmp];
                                    UVSensor_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        #else
                            UVSensor_ClearTxInterruptSource(UVSensor_INTR_TX_EMPTY);
                        #endif /* (UVSensor_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(UVSensor_CHECK_INTR_TX_MASKED(UVSensor_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            UVSensor_mstrWrBufIndex = UVSensor_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(UVSensor_CHECK_I2C_MODE_NO_STOP(UVSensor_mstrControl))
                    {
                        /* On-going transaction is suspended: the ReStart is generated by the API request */
                        UVSensor_mstrStatus |= (uint16) (UVSensor_I2C_MSTAT_XFER_HALT |
                                                                 UVSensor_GET_I2C_MSTAT_CMPLT);

                        UVSensor_state = UVSensor_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
                        ...
                        */
                        UVSensor_I2C_MASTER_GENERATE_STOP;
                    }
                }
            }
    ...
    } /* (UVSensor_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(UVSensor_CHECK_I2C_FSM_SLAVE)
    {
        #if(UVSensor_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(UVSensor_CHECK_INTR_SLAVE_MASKED(UVSensor_INTR_SLAVE_I2C_NACK))
            {
                UVSensor_ClearSlaveInterruptSource(UVSensor_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (UVSensor_GET_TX_FIFO_ENTRIES + UVSensor_GET_TX_FIFO_SR_VALID);

                if(UVSensor_slOverFlowCount > diffCount) /* Overflow */
                {
                    UVSensor_slStatus |= (uint8) UVSensor_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    UVSensor_slRdBufIndexTmp -= (diffCount - UVSensor_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                UVSensor_slRdBufIndex = UVSensor_slRdBufIndexTmp;
                ...
                UVSensor_slStatus &= (uint8) ~UVSensor_I2C_SSTAT_RD_BUSY;
                UVSensor_slStatus |= (uint8)  UVSensor_I2C_SSTAT_RD_CMPLT;
                UVSensor_state     =  UVSensor_I2C_FSM_IDLE;
                
            #ifdef UVSensor_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                UVSensor_I2C_SlaveCompleteCallback();
            #endif /* UVSensor_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(UVSensor_CHECK_INTR_SLAVE_MASKED(UVSensor_INTR_SLAVE_I2C_WRITE_STOP))
            {
                UVSensor_ClearSlaveInterruptSource(UVSensor_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (UVSensor_I2C_CTRL_REG & UVSensor_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != UVSensor_GET_RX_FIFO_ENTRIES)
                    {
                        #if(UVSensor_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == UVSensor_GET_RX_FIFO_ENTRIES) &&
                               (UVSensor_CHECK_INTR_SLAVE_MASKED(UVSensor_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        UVSensor_slWrBufPtr[UVSensor_slWrBufIndex] = (uint8) UVSensor_RX_FIFO_RD_REG;
                        UVSensor_slWrBufIndex++;
                    }

                    UVSensor_DISABLE_SLAVE_AUTO_DATA;
                }

                if(UVSensor_CHECK_INTR_RX(UVSensor_INTR_RX_OVERFLOW))
                {
                    UVSensor_slStatus |= (uint8) UVSensor_I2C_SSTAT_WR_OVFL;
                }
                ...
                UVSensor_slStatus &= (uint8) ~UVSensor_I2C_SSTAT_WR_BUSY;
                UVSensor_slStatus |= (uint8)  UVSensor_I2C_SSTAT_WR_CMPLT;
                UVSensor_state     =  UVSensor_I2C_FSM_IDLE;
            ...
            */
        ...
        #endif /* (UVSensor_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                UVSensor_ClearI2CExtClkInterruptSource(UVSensor_INTR_I2C_EC_WAKE_UP);

                #if (UVSensor_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != UVSensor_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = UVSensor_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) UVSensor_RX_FIFO_RD_REG;
                        response = UVSensor_I2C_ACK_ADDR;
                    }
            ...
            #if (UVSensor_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == UVSensor_I2C_NAK_ADDR)
                {
                #if (!UVSensor_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    UVSensor_DISABLE_INTR_SLAVE(UVSensor_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!UVSensor_CY_SCBIP_V0) */
                    ...
                    UVSensor_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (UVSensor_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        UVSensor_slRdBufIndexTmp = UVSensor_slRdBufIndex;

                        /* Start master reading */
                        UVSensor_slStatus |= (uint8) UVSensor_I2C_SSTAT_RD_BUSY;
                        UVSensor_state     = UVSensor_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (UVSensor_slWrBufSize - UVSensor_slWrBufIndex);

                    #if (UVSensor_CY_SCBIP_V0)
                        if(diffCount < UVSensor_I2C_FIFO_SIZE)
                        /* Receive data: byte-by-byte */
                        {
                            UVSensor_SetRxInterruptMode(UVSensor_INTR_RX_NOT_EMPTY);
                        }
                        else
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == UVSensor_I2C_FIFO_SIZE)
                            {
                                ...
                                UVSensor_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                UVSensor_ENABLE_SLAVE_AUTO_DATA_ACK;
                                UVSensor_SetRxInterruptMode(UVSensor_INTR_RX_FULL);
                            }
                        }
                        ...
                        /* Start master reading */
                        UVSensor_slStatus |= (uint8) UVSensor_I2C_SSTAT_WR_BUSY;
                        UVSensor_state     = UVSensor_I2C_FSM_SL_WR;
                    }
                    ...
                    UVSensor_ENABLE_INTR_SLAVE(UVSensor_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!UVSensor_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    UVSensor_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(UVSensor_CHECK_INTR_RX_MASKED(UVSensor_INTR_RX_FULL))
            {
                ...
                diffCount =  UVSensor_slWrBufSize -
                            (UVSensor_slWrBufIndex + UVSensor_I2C_FIFO_SIZE);

                if(diffCount > UVSensor_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = UVSensor_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    UVSensor_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                UVSensor_ClearRxInterruptSource(UVSensor_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = UVSensor_RX_FIFO_RD_REG;

                if(UVSensor_slWrBufIndex < UVSensor_slWrBufSize)
                {
                    UVSensor_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    UVSensor_slWrBufPtr[UVSensor_slWrBufIndex] = (uint8) diffCount;
                    UVSensor_slWrBufIndex++;
                }
                ...
                UVSensor_ClearRxInterruptSource(UVSensor_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(UVSensor_CHECK_INTR_TX_MASKED(UVSensor_INTR_TX_EMPTY))
            {
                while(UVSensor_I2C_FIFO_SIZE != UVSensor_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(UVSensor_slRdBufIndexTmp < UVSensor_slRdBufSize)
                    /* Data from buffer */
                    {
                        UVSensor_TX_FIFO_WR_REG = (uint32) UVSensor_slRdBufPtr[UVSensor_slRdBufIndexTmp];
                        UVSensor_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        UVSensor_TX_FIFO_WR_REG = UVSensor_I2C_SLAVE_OVFL_RETURN;

                        if(UVSensor_slOverFlowCount <= UVSensor_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            UVSensor_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef UVSensor_I2C_ISR_EXIT_CALLBACK
    UVSensor_I2C_ISR_ExitCallback();
#endif /* UVSensor_I2C_ISR_EXIT_CALLBACK */
