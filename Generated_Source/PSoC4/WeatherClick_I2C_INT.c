...
/*******************************************************************************
* Function Name: WeatherClick_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(WeatherClick_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef WeatherClick_I2C_ISR_ENTRY_CALLBACK
    WeatherClick_I2C_ISR_EntryCallback();
#endif /* WeatherClick_I2C_ISR_ENTRY_CALLBACK */

#if (WeatherClick_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = WeatherClick_I2C_ACK_ADDR;
    ...
    if(NULL != WeatherClick_customIntrHandler)
    {
        WeatherClick_customIntrHandler();
    }

    if(WeatherClick_CHECK_INTR_I2C_EC_MASKED(WeatherClick_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(WeatherClick_CHECK_I2C_FSM_MASTER)
    {
        #if(WeatherClick_I2C_MASTER)
        {
            ...
            */
            if(WeatherClick_CHECK_INTR_MASTER_MASKED(WeatherClick_INTR_MASTER_I2C_BUS_ERROR))
            {
                WeatherClick_mstrStatus |= (uint16) (WeatherClick_I2C_MSTAT_ERR_XFER |
                                                         WeatherClick_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = WeatherClick_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(WeatherClick_CHECK_INTR_MASTER_MASKED(WeatherClick_INTR_MASTER_I2C_ARB_LOST))
            {
                WeatherClick_mstrStatus |= (uint16) (WeatherClick_I2C_MSTAT_ERR_XFER |
                                                         WeatherClick_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = WeatherClick_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(WeatherClick_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(WeatherClick_CHECK_I2C_MASTER_CMD(WeatherClick_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    WeatherClick_mstrStatus |= (uint16) (WeatherClick_I2C_MSTAT_ERR_XFER |
                                                             WeatherClick_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                WeatherClick_mstrStatus |= (uint16) WeatherClick_GET_I2C_MSTAT_CMPLT;

                #if(WeatherClick_I2C_MULTI_MASTER_SLAVE)
                {
                    if(WeatherClick_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(WeatherClick_CHECK_I2C_MASTER_CMD(WeatherClick_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            WeatherClick_I2C_MASTER_CLEAR_START;

                            endTransfer = WeatherClick_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!WeatherClick_CHECK_INTR_SLAVE_MASKED(WeatherClick_INTR_SLAVE_I2C_BUS_ERROR))
                               && WeatherClick_CHECK_INTR_MASTER_MASKED(WeatherClick_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = WeatherClick_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            WeatherClick_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            WeatherClick_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            WeatherClick_state = WeatherClick_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (WeatherClick_CHECK_I2C_FSM_SLAVE) */
    {
        #if(WeatherClick_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or WeatherClick_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(WeatherClick_CHECK_INTR_SLAVE_MASKED(WeatherClick_INTR_SLAVE_I2C_BUS_ERROR |
                                                        WeatherClick_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(WeatherClick_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    WeatherClick_slStatus &= (uint8) ~WeatherClick_I2C_SSTAT_RD_BUSY;
                    WeatherClick_slStatus |= (uint8) (WeatherClick_I2C_SSTAT_RD_ERR |
                                                          WeatherClick_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    WeatherClick_slStatus &= (uint8) ~WeatherClick_I2C_SSTAT_WR_BUSY;
                    WeatherClick_slStatus |= (uint8) (WeatherClick_I2C_SSTAT_WR_ERR |
                                                          WeatherClick_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(WeatherClick_CHECK_I2C_FSM_MASTER)
    {
        #if(WeatherClick_I2C_MASTER)
        {
            ...
            */
            if(WeatherClick_CHECK_INTR_MASTER_MASKED(WeatherClick_INTR_MASTER_I2C_STOP))
            {
                WeatherClick_ClearMasterInterruptSource(WeatherClick_INTR_MASTER_I2C_STOP);
            ...
            {
                if(WeatherClick_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(WeatherClick_CHECK_INTR_MASTER_MASKED(WeatherClick_INTR_MASTER_I2C_NACK))
                    {
                        WeatherClick_ClearMasterInterruptSource(WeatherClick_INTR_MASTER_I2C_NACK);

                        WeatherClick_mstrStatus |= (uint16) (WeatherClick_I2C_MSTAT_ERR_XFER |
                                                                 WeatherClick_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(WeatherClick_CHECK_I2C_FSM_RD) /* Reading */
                        {
                            WeatherClick_state = WeatherClick_I2C_FSM_MSTR_RD_DATA;
                        }
                        else /* Writing */
                        {
                            WeatherClick_state = WeatherClick_I2C_FSM_MSTR_WR_DATA;
                            if(0u != WeatherClick_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                WeatherClick_SetTxInterruptMode(WeatherClick_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(WeatherClick_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(WeatherClick_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(WeatherClick_CHECK_INTR_RX_MASKED(WeatherClick_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  WeatherClick_mstrRdBufSize -
                                        (WeatherClick_mstrRdBufIndex + WeatherClick_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > WeatherClick_I2C_FIFO_SIZE)
                            {
                                diffCount = WeatherClick_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    WeatherClick_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                WeatherClick_mstrRdBufPtr[WeatherClick_mstrRdBufIndex] = (uint8)
                                                                                        WeatherClick_RX_FIFO_RD_REG;
                                WeatherClick_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(WeatherClick_CHECK_INTR_RX_MASKED(WeatherClick_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            WeatherClick_mstrRdBufPtr[WeatherClick_mstrRdBufIndex] = (uint8) WeatherClick_RX_FIFO_RD_REG;
                            WeatherClick_mstrRdBufIndex++;

                            if(WeatherClick_mstrRdBufIndex < WeatherClick_mstrRdBufSize)
                            {
                                WeatherClick_I2C_MASTER_GENERATE_ACK;
                            }
                            else
                            {
                               endTransfer = WeatherClick_I2C_CMPLT_ANY_TRANSFER;
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(WeatherClick_CHECK_INTR_MASTER_MASKED(WeatherClick_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            WeatherClick_mstrWrBufIndexTmp -= (WeatherClick_GET_TX_FIFO_ENTRIES +
                                                                   WeatherClick_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            WeatherClick_mstrWrBufIndex = WeatherClick_mstrWrBufIndexTmp;

                            WeatherClick_mstrStatus |= (uint16) (WeatherClick_I2C_MSTAT_ERR_XFER |
                                                                     WeatherClick_I2C_MSTAT_ERR_SHORT_XFER);

                            WeatherClick_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(WeatherClick_CHECK_INTR_TX_MASKED(WeatherClick_INTR_TX_EMPTY))
                        {
                            while(WeatherClick_I2C_FIFO_SIZE != WeatherClick_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(WeatherClick_mstrWrBufIndexTmp < WeatherClick_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(WeatherClick_mstrWrBufIndexTmp == (WeatherClick_mstrWrBufSize - 1u))
                                    {
                                        WeatherClick_ClearTxInterruptSource(WeatherClick_INTR_TX_UNDERFLOW);
                                        WeatherClick_SetTxInterruptMode(WeatherClick_INTR_TX_UNDERFLOW);
                                    }
                                 #endif /* (!WeatherClick_CY_SCBIP_V0) */

                                    /* Put data into TX FIFO */
                                    WeatherClick_TX_FIFO_WR_REG = (uint32) WeatherClick_mstrWrBufPtr[WeatherClick_mstrWrBufIndexTmp];
                                    WeatherClick_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        #else
                            WeatherClick_ClearTxInterruptSource(WeatherClick_INTR_TX_EMPTY);
                        #endif /* (WeatherClick_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(WeatherClick_CHECK_INTR_TX_MASKED(WeatherClick_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            WeatherClick_mstrWrBufIndex = WeatherClick_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(WeatherClick_CHECK_I2C_MODE_NO_STOP(WeatherClick_mstrControl))
                    {
                        /* On-going transaction is suspended: the ReStart is generated by the API request */
                        WeatherClick_mstrStatus |= (uint16) (WeatherClick_I2C_MSTAT_XFER_HALT |
                                                                 WeatherClick_GET_I2C_MSTAT_CMPLT);

                        WeatherClick_state = WeatherClick_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
                        ...
                        */
                        WeatherClick_I2C_MASTER_GENERATE_STOP;
                    }
                }
            }

        } /* (WeatherClick_I2C_MASTER) */
        #endif

    } /* (WeatherClick_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(WeatherClick_CHECK_I2C_FSM_SLAVE)
    {
        #if(WeatherClick_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(WeatherClick_CHECK_INTR_SLAVE_MASKED(WeatherClick_INTR_SLAVE_I2C_NACK))
            {
                WeatherClick_ClearSlaveInterruptSource(WeatherClick_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (WeatherClick_GET_TX_FIFO_ENTRIES + WeatherClick_GET_TX_FIFO_SR_VALID);

                if(WeatherClick_slOverFlowCount > diffCount) /* Overflow */
                {
                    WeatherClick_slStatus |= (uint8) WeatherClick_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    WeatherClick_slRdBufIndexTmp -= (diffCount - WeatherClick_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                WeatherClick_slRdBufIndex = WeatherClick_slRdBufIndexTmp;
                ...
                WeatherClick_slStatus &= (uint8) ~WeatherClick_I2C_SSTAT_RD_BUSY;
                WeatherClick_slStatus |= (uint8)  WeatherClick_I2C_SSTAT_RD_CMPLT;
                WeatherClick_state     =  WeatherClick_I2C_FSM_IDLE;
                
            #ifdef WeatherClick_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                WeatherClick_I2C_SlaveCompleteCallback();
            #endif /* WeatherClick_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(WeatherClick_CHECK_INTR_SLAVE_MASKED(WeatherClick_INTR_SLAVE_I2C_WRITE_STOP))
            {
                WeatherClick_ClearSlaveInterruptSource(WeatherClick_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (WeatherClick_I2C_CTRL_REG & WeatherClick_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != WeatherClick_GET_RX_FIFO_ENTRIES)
                    {
                        #if(WeatherClick_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == WeatherClick_GET_RX_FIFO_ENTRIES) &&
                               (WeatherClick_CHECK_INTR_SLAVE_MASKED(WeatherClick_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        WeatherClick_slWrBufPtr[WeatherClick_slWrBufIndex] = (uint8) WeatherClick_RX_FIFO_RD_REG;
                        WeatherClick_slWrBufIndex++;
                    }

                    WeatherClick_DISABLE_SLAVE_AUTO_DATA;
                }

                if(WeatherClick_CHECK_INTR_RX(WeatherClick_INTR_RX_OVERFLOW))
                {
                    WeatherClick_slStatus |= (uint8) WeatherClick_I2C_SSTAT_WR_OVFL;
                }
                ...
                WeatherClick_slStatus &= (uint8) ~WeatherClick_I2C_SSTAT_WR_BUSY;
                WeatherClick_slStatus |= (uint8)  WeatherClick_I2C_SSTAT_WR_CMPLT;
                WeatherClick_state     =  WeatherClick_I2C_FSM_IDLE;
            ...
            */
        ...
        #endif /* (WeatherClick_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                WeatherClick_ClearI2CExtClkInterruptSource(WeatherClick_INTR_I2C_EC_WAKE_UP);

                #if (WeatherClick_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != WeatherClick_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = WeatherClick_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) WeatherClick_RX_FIFO_RD_REG;
                        response = WeatherClick_I2C_ACK_ADDR;
                    }
            ...
            #if (WeatherClick_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == WeatherClick_I2C_NAK_ADDR)
                {
                #if (!WeatherClick_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    WeatherClick_DISABLE_INTR_SLAVE(WeatherClick_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!WeatherClick_CY_SCBIP_V0) */
                    ...
                    WeatherClick_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (WeatherClick_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        WeatherClick_slRdBufIndexTmp = WeatherClick_slRdBufIndex;

                        /* Start master reading */
                        WeatherClick_slStatus |= (uint8) WeatherClick_I2C_SSTAT_RD_BUSY;
                        WeatherClick_state     = WeatherClick_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (WeatherClick_slWrBufSize - WeatherClick_slWrBufIndex);

                    #if (WeatherClick_CY_SCBIP_V0)
                        if(diffCount < WeatherClick_I2C_FIFO_SIZE)
                        /* Receive data: byte-by-byte */
                        {
                            WeatherClick_SetRxInterruptMode(WeatherClick_INTR_RX_NOT_EMPTY);
                        }
                        else
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == WeatherClick_I2C_FIFO_SIZE)
                            {
                                ...
                                WeatherClick_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                WeatherClick_ENABLE_SLAVE_AUTO_DATA_ACK;
                                WeatherClick_SetRxInterruptMode(WeatherClick_INTR_RX_FULL);
                            }
                        }
                        ...
                        /* Start master reading */
                        WeatherClick_slStatus |= (uint8) WeatherClick_I2C_SSTAT_WR_BUSY;
                        WeatherClick_state     = WeatherClick_I2C_FSM_SL_WR;
                    }
                    ...
                    WeatherClick_ENABLE_INTR_SLAVE(WeatherClick_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!WeatherClick_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    WeatherClick_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(WeatherClick_CHECK_INTR_RX_MASKED(WeatherClick_INTR_RX_FULL))
            {
                ...
                diffCount =  WeatherClick_slWrBufSize -
                            (WeatherClick_slWrBufIndex + WeatherClick_I2C_FIFO_SIZE);

                if(diffCount > WeatherClick_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = WeatherClick_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    WeatherClick_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                WeatherClick_ClearRxInterruptSource(WeatherClick_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = WeatherClick_RX_FIFO_RD_REG;

                if(WeatherClick_slWrBufIndex < WeatherClick_slWrBufSize)
                {
                    WeatherClick_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    WeatherClick_slWrBufPtr[WeatherClick_slWrBufIndex] = (uint8) diffCount;
                    WeatherClick_slWrBufIndex++;
                }
                ...
                WeatherClick_ClearRxInterruptSource(WeatherClick_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(WeatherClick_CHECK_INTR_TX_MASKED(WeatherClick_INTR_TX_EMPTY))
            {
                while(WeatherClick_I2C_FIFO_SIZE != WeatherClick_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(WeatherClick_slRdBufIndexTmp < WeatherClick_slRdBufSize)
                    /* Data from buffer */
                    {
                        WeatherClick_TX_FIFO_WR_REG = (uint32) WeatherClick_slRdBufPtr[WeatherClick_slRdBufIndexTmp];
                        WeatherClick_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        WeatherClick_TX_FIFO_WR_REG = WeatherClick_I2C_SLAVE_OVFL_RETURN;

                        if(WeatherClick_slOverFlowCount <= WeatherClick_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            WeatherClick_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef WeatherClick_I2C_ISR_EXIT_CALLBACK
    WeatherClick_I2C_ISR_ExitCallback();
#endif /* WeatherClick_I2C_ISR_EXIT_CALLBACK */
