/***************************************************************************//**
* \file AirQuality2Click_I2C_BOOT.c
* \version 4.0
*
* \brief
...
static uint8 AirQuality2Click_slReadBuf[AirQuality2Click_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 AirQuality2Click_slWriteBuf[AirQuality2Click_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 AirQuality2Click_applyBuffer;
...
static void AirQuality2Click_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: AirQuality2Click_I2CCyBtldrCommStart
****************************************************************************//**
*
...
*
* \globalvars
*  AirQuality2Click_applyBuffer - the flag to release the buffer with a response
*  to be read by the host.
*
*******************************************************************************/
void AirQuality2Click_I2CCyBtldrCommStart(void)
{
    AirQuality2Click_I2CSlaveInitWriteBuf(AirQuality2Click_slWriteBuf, AirQuality2Click_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    AirQuality2Click_I2CSlaveInitReadBuf (AirQuality2Click_slReadBuf, 0u);

    AirQuality2Click_SetCustomInterruptHandler(&AirQuality2Click_I2CResposeInsert);
    AirQuality2Click_applyBuffer = 0u;

    AirQuality2Click_Start();
}


/*******************************************************************************
* Function Name: AirQuality2Click_I2CCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: AirQuality2Click_I2CCyBtldrCommReset
****************************************************************************//**
*
...
*
* \globalvars
*  AirQuality2Click_slRdBufSize - used to store slave read buffer size.
*  AirQuality2Click_slRdBufIndex - used to store the current index within the
*  slave read buffer.
...
void AirQuality2Click_I2CCyBtldrCommReset(void)
{
    ...
    AirQuality2Click_slRdBufSize = 0u;
...
/*******************************************************************************
* Function Name: AirQuality2Click_I2CCyBtldrCommRead
****************************************************************************//**
*
...
* \globalvars
*  AirQuality2Click_slStatus  - used to store current status of I2C slave.
*  AirQuality2Click_slWriteBuf - used to store received command.
*  AirQuality2Click_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus AirQuality2Click_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (AirQuality2Click_I2C_SSTAT_WR_CMPLT & AirQuality2Click_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) AirQuality2Click_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) AirQuality2Click_slWriteBuf,
                                              AirQuality2Click_BYTES_TO_COPY(*count, size));

                status = CYRET_SUCCESS;
                break;
            }

            CyDelay(AirQuality2Click_WAIT_1_MS);
            --timeoutMs;
        }
    }
...
/*******************************************************************************
* Function Name: AirQuality2Click_I2CCyBtldrCommWrite
****************************************************************************//**
*
...
*
* \globalvars
*  AirQuality2Click_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
cystatus AirQuality2Click_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) AirQuality2Click_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        AirQuality2Click_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: AirQuality2Click_I2CResposeInsert
****************************************************************************//**
*
...
static void AirQuality2Click_I2CResposeInsert(void)
{
    if (AirQuality2Click_CHECK_INTR_SLAVE_MASKED(AirQuality2Click_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (AirQuality2Click_CHECK_I2C_STATUS(AirQuality2Click_I2C_STATUS_S_READ))
        {
            ...
            if (0u != AirQuality2Click_applyBuffer)
            {
                AirQuality2Click_slRdBufSize  = AirQuality2Click_applyBuffer;
                AirQuality2Click_slRdBufIndex = 0u;
                AirQuality2Click_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (AirQuality2Click_slRdBufIndex != AirQuality2Click_slRdBufSize)
            {
                AirQuality2Click_slRdBufIndex = AirQuality2Click_slRdBufSize;
            }
        }
    }
}
